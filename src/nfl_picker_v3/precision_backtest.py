...
def prepare_precision_backtest(df: pd.DataFrame) -> pd.DataFrame:
    """
    Prepare historical V3.1 predictions for precision-layer testing.

    IMPORTANT:
    Historical model agreement is used only if independent
    Original V3 and V3.1 ML picks actually exist.
    """

    out = df.copy()

    # ---------------------------------------------------------
    # Identify historical prediction / result columns
    # ---------------------------------------------------------
    ...
    # ---------------------------------------------------------
    # Historical model agreement
    # Never fabricate agreement.
    # ---------------------------------------------------------
    ...
    # ---------------------------------------------------------
    # Build confidence / quality fields when possible
    # ---------------------------------------------------------
...
def precision_summary(df: pd.DataFrame) -> pd.DataFrame:
    ...
    order = ["BEST PICK", "PLAY", "LEAN", "PASS"]

    for label in order:

        group = df[
            df["precision_recommendation"].eq(label)
        ]
